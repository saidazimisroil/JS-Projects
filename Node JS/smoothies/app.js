...
app.get("/food", requireAuth, async (req, res) => {
  const productsData = await Product.find({ category: "food" });
  res.render("products", { products: productsData });
});
app.get("/drinks", requireAuth, async (req, res) => {
  const productsData = await Product.find({ category: "drinks" });
  res.render("products", { products: productsData });
});
app.get("/product/:id", requireAuth, async (req, res) => {
  const productsData = await Product.find({});
  res.render("about", { products: productsData, id: req.params.id });
});
// new product
app.get("/newproduct", requireAuth, (req, res) => res.render("new"));
app.post("/newproduct", requireAuth, upload.single("image"), async (req, res) => {
...
function handleErrors(err) {
  ...
  if (err.name === "ValidationError" || err.message.includes("jpg and png files are supported")) {
